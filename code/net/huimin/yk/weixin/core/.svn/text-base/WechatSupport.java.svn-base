...
public abstract class WechatSupport {
	...
	private String response(){
		...
		try {
			...
			//设置
			jaxbParser.setCdataNode(new String[]{"Content"});
			result = jaxbParser.toXML(wechatResponse);
			if("text".equals(this.wechatResponse.getMsgType())){
				String qian = result.substring(0,result.indexOf("<![CDATA["));
				String hou = result.substring(result.indexOf("]]>"));
				result = qian + "<![CDATA[" + wechatResponse.getContent() + hou;
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
