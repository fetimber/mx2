		...
		WmallAPP.controller("shopcart_Controller", ["$scope", "shopcart_Service", "$filter", function($scope, shopcart_Service, $filter){
			...
			$scope.selectAll = function(thi, evt){
				...
				$scope.data.forEach(function(v, k){
					v.checked = v.BARGAIN_NUMBER && $scope.checkAll;
				});
			...
			$scope.changeSum = function(thi, evt, type, idx){
				var ci = $scope.data[idx], _Sum = ci.BARGAIN_NUMBER;
				if (1 == ci.status) {
				    alert("该商品已下架了哦~");
				    return;
				}
				switch(type){
					case "-":
						_Sum --;
						$scope.data[idx].BARGAIN_NUMBER = Math.max(1, _Sum);
					break;
					case "+":
						_Sum ++;
						if (ci.STOCK <= ci.BARGAIN_NUMBER) {
		                    alert( "商品购买数量不能超过库存数哦~该商品最多可购买"+(ci.STOCK)+"件" );
		                    return;
		                }
						$scope.data[idx].BARGAIN_NUMBER = _Sum;
					break;
				}
				if(activity.length){
					alert("activity");
			...
			function getTotal(from){
				...
				$scope.selected.forEach(function(ci, i){
					...
					t_s +=ci.BARGAIN_NUMBER;
					t_a += ci.BARGAIN_NUMBER*parseFloat(ci.GOODSPRICE);
				});
				$scope.totleSum = t_s;
			...
			$scope.checkout = function(thi, evt){
		        ...
		        for (var i=0, ci; ci = $scope.selected[i]; i++) {
		            ...
		            _data.push([ci.GOODS_ID, ci.BARGAIN_NUMBER]);
		            jsonData.push({ productId: ci.GOODS_ID, count: ci.BARGAIN_NUMBER });
		        ...
		        if (tip_flag) {
		            ...
		            formData.append("buy_list", ProductJson);
		            var l = loading();
		            new ajax3({
			...
			$scope.goToDetail = function(thi, evt, ci){
				location.href = "weixin.do?method=getMobileGoodsDetail&id="+ci.GOODS_ID;//ci.DetailUrl;
			}
			//删除失效商品
