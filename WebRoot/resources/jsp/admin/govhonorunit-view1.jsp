...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			function _forward_page(page){
			    $("#base_frm").attr('action','admin/govhonorunit!view1');
				$("#page_current").val(page);
				$("#base_frm").submit();
			}
			
			function exportData(){
				$("#base_frm").attr('action','admin/govhonorunit!export?checkFlag=1');
				$("#base_frm").submit();
			}
			
			function delete_honor(id,name){
			    $.dialog.confirm("你确定要删除此单位荣誉[" + name + "]吗？<br /><br />删除后，将不再显示此信息，请谨慎操作。", function(){
			...
			function checkHonor(honorrid,name,flag){
			     ...
			     $.dialog.confirm(tip, function(){
				   ...
				   $.get("admin/govhonorunit!check",{"honor.id":honorrid,
				    "honor.checkFlag":checkflag,"honor.checkDesc":remark},function(data){
					   eval("data=" + data);
					   if(data.result){
	...
	<body>
		<div class="home">
			<img src="resources/images/icon_home.png" width="20" height="20" />荣誉单位库&nbsp;>&nbsp;单位荣誉审核
		</div>
		<div class="m_w">
		 
			<!--检索部分-->
			<s:form action="govhonorunit!view1" namespace="/admin" id="base_frm">
