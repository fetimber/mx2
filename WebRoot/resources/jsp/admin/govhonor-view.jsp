	...
	<head>
		...
		<script type="text/javascript">
			...
			function _forward_page(page){
			    $("#base_frm").attr('action','admin/govhonor!view');
				$("#page_current").val(page);
				$("#base_frm").submit();
			}
			
			function exportData(){
				$("#base_frm").attr('action','admin/govhonor!export?checkFlag=0');
				$("#base_frm").submit();
			}
...
<!-- 		    <div class="p_tpc" onclick="showSearch();" style="cursor: pointer;">搜索条件:</div> -->
			<!--检索部分-->
			<s:form action="govhonor!view" namespace="/admin" id="base_frm">
			...
			<div class="tab_check" id="search" > 
				   ...
				   <s:select name="query.check" list="#{'':'审核状态','-2':'未提交','0':'未审核',1:'审核通过',2:'审核拒绝'}" ></s:select> <span>&nbsp;</span> <span>&nbsp;</span>
					    ...
					    <input  type="button" value="导入EXCEL" class="btn_search importexcel" style="margin-left:10px;margin-top:-3px;align:left" />
					    <input  type="button" value="导出" class="btn_search" style="margin-left:10px;margin-top:-3px;align:left" onclick="exportData();"/>
				        <input  type="button" value="模板下载" class="btn_search" style="margin-left:10px;margin-top:-3px;align:left" onclick="location.href='<%=path%>/template/honordata.xls'"/> 
						 </td>
						</tr>
					</table>
				...
				<s:iterator value="#request.page.beans" var="item" status="st">
					<tr <s:if test="#st.even">class="inter"</s:if>>
						...
						<td align="center" valign="top">
                              ...
                              |  <a href="admin/govhonor!add?honor.secId=<s:property value="#item.secId" />">修改</a>
